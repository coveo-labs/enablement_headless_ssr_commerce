...
export default function FacetSearch({
  ...
  return (
    <div className="flex flex-col mb-2 w-full" role="search">
      <label className="text-sm font-medium mb-1" htmlFor={`facetSearchInput-${facetId}`}>
        Filter values:
      </label>
      <div className="flex w-full items-center">
        <input
          className="flex-grow min-w-0 text-sm px-2 py-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {facetSearch.isLoading ? (
          <div className={`${controlStyles} ml-2`}>
            <div className="w-4 h-4 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin" />
          </div>
        ) : (
          facetSearch.query && (
            <button
              className={`${controlStyles} ${buttonHoverStyles} ml-2`}
